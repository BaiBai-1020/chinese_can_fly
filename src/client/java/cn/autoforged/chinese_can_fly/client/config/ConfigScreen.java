...
public class ConfigScreen extends Screen {
	...
	private void drawLabel(GuiGraphicsExtractor g, String key, EditBox field, int color, int lh) {
		if (field == null) return;
		String txt = Component.translatable("screen." + ExampleMod.MOD_ID + ".config." + key).getString();
		g.text(this.font, txt, field.getX() - LABEL_WIDTH - 4, field.getY() + (WIDGET_HEIGHT - lh) / 2 + lh, color, false);
	}

	@Override public void onClose() { saveAllFields(); this.minecraft.gui.setScreen(parent); }
}
